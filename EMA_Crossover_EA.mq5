...
//+------------------------------------------------------------------+
#property copyright "EMA Crossover EA v1.9"
...
int OnInit()
{
   ...
   Print("EMA Crossover EA v1.9 initialized | EMA ", EMA_Mid_Period, "/", EMA_Slow_Period,
         "/", EMA_Exit_Period,
...
void ExecuteBuy(double entryClose, double ema250)
{
   ...
   // Use distance to EMA 250 for lot calculation
   double riskDist = MathAbs(price - ema250);
   if(riskDist < _Point) riskDist = 100 * _Point;  // min 100 points fallback

   double slForCalc = price - riskDist;
   double lot = CalcLotFromRisk(price, slForCalc, false);

   if(g_trade.Buy(lot, _Symbol, price, 0, 0,
      StringFormat("EMA%d/%d RSI Buy|Risk$%.0f", EMA_Mid_Period, EMA_Slow_Period, Risk_Amount)))
   {
      ulong ticket = g_trade.ResultOrder();
      AddTrade(ticket, -1);
      Print(StringFormat("BUY #%d: Price=%.2f Lot=%.2f EMA250=%.2f RiskDist=%.1f pts Risk=$%.2f",
            ticket, price, lot, ema250, riskDist / _Point, Risk_Amount));
   }
   else
...
void ExecuteSell(double entryClose, double ema250)
{
   ...
   double slForCalc = price + riskDist;
   double lot = CalcLotFromRisk(price, slForCalc, true);

   if(g_trade.Sell(lot, _Symbol, price, 0, 0,
      StringFormat("EMA%d/%d RSI Sell|Risk$%.0f", EMA_Mid_Period, EMA_Slow_Period, Risk_Amount)))
   {
      ulong ticket = g_trade.ResultOrder();
      AddTrade(ticket, 1);
      Print(StringFormat("SELL #%d: Price=%.2f Lot=%.2f EMA250=%.2f RiskDist=%.1f pts Risk=$%.2f",
            ticket, price, lot, ema250, riskDist / _Point, Risk_Amount));
   }
   else
